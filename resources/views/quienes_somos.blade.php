@extends('layouts.header')

@section('titulo')
{{ 'Quienes Somos | TEIPRO' }}
@endsection

@section('menu1'){{ '' }} @endsection
@section('menu2'){{ 'active' }} @endsection
@section('menu3'){{ '' }} @endsection
@section('menu4'){{ '' }} @endsection
@section('menu5'){{ '' }} @endsection
@section('menu6'){{ '' }} @endsection

@section('contenido')
<div class="container-fluid row">
	<div class="text-center tituloCentrado">
		Quiénes Somos
	</div>
	<div class="col-sm-6">
		<div class="textoTitulos">Nuestra Organización</div>
		<div class="textoContenido1">
			<br>
			Somos una red de especialistas en tecnología que creemos firmemente en la innovación y la técnica como regalos de Dios para servir a nuestros clientes, traer prosperidad a nuestros aliados  y generar progreso a nuestro país. 

			<strong>TEIPRO COLOMBIA</strong> empezó a operar en 2014 a través de los servicios profesionales en ingeniería electrónica prestados por su fundador quien detectó la necesidad de muchas empresas de contar con aliados tecnológicos firmes y confiables que le ofrezcan soluciones de pequeña escala en automatización, mantenimiento electrónico y diseño electrónico a la medida. En 2016, gracias a varias alianzas con expertos y proveedores del sector tecnológico logramos consolidarnos como empresa  ofreciendo nuestros servicios y soluciones a grupos de investigación, empresas industriales y Pymes de base tecnológica. 
		</div>
	</div>
	<div class="col-sm-6">
		<div class="textoTitulos">Nuestra Misión</div>
		<div class="textoContenido1">
			<br>
			<strong>TEIPRO COLOMBIA</strong> es una empresa dedicada a contribuir al desarrollo de Colombia a través de llevar las mejores soluciones tecnológicas a nuestros clientes en las áreas de mantenimiento industrial, automatización, inteligencia geográfica y tecnología electrónica a la medida.
		</div> 
		<div class="textoTitulos">Nuestra Visión</div>
		<div class="textoContenido1">
			<br>
			En 2019, <strong>TEIPRO COLOMBIA</strong> habrá consolidado una base empresarial de clientes a nivel nacional con alto grado de satisfacción, recompra y casos de éxito en las áreas de mantenimiento industrial, automatización y TIC, así mismo habremos aportado a la sociedad soluciones tecnológicas en las áreas de inteligencia geográfica, movilidad inteligente y/o electrónica.
		</div>
	</div>
	<div class="clearfix visible"></div>
	<div class="col-sm-6">
		<div class="textoTitulos">Declaraciones TEIPRO Colombia SAS</div>
		<div class="textoContenido1">
			<br>
			<strong>Prosperidad para nuestro equipo</strong><br>
			Entendemos la prosperidad como la oportunidad de trabajar en lo que nos
			apasiona y recibir una remuneración justa y suficiente para nuestras
			necesidades y sueños.<br>
			<br>
			<strong>Servir a nuestros clientes</strong><br>
			Creemos firmemente en que nuestros clientes no son empresas, nuestros
			clientes son las personas que les dan vida y que necesitan la tranquilidad de
			encontrar aliados confiables que mediante orden y cumplimiento pueda
			proveerles buenos productos y servicios a precios justos, enfocados en
			conservar sus activos.<br>
			<br>
			<strong>Progreso para nuestro País</strong><br>		
			Soñamos con una Colombia próspera y justa, con un desarrollo amigable
			con la naturaleza. Nuestro aporte para este anhelo es ser una organización
			transparente en el manejo de nuestras obligaciones financieras con
			nuestros trabajadores, nuestros proveedores y nuestros impuestos. Por otro
			lado, estamos comprometidos en que nuestras soluciones siempre van a
			traer beneficios ambientales para nuestro planeta.
		</div>
	</div>
	<div class="col-sm-6">
		<div class="textoTitulos">Nuestros Valores</div>
		<div class="textoContenido1">
			<br>
			<strong>Confianza:</strong> nuestro mayor patrimonio es la credibilidad que nuestros
			clientes tienen en nosotros, por esta razón nos esforzamos en
			entregarle la mejor oferta en calidad, competitiva y que va a resolver
			exitosamente su aplicación.<br>
			<br>
			<strong>Compromiso:</strong> En Teipro Colombia entendemos que nuestro cliente
			son las personas, quienes necesitan tener la tranquilidad de contar
			con aliados firmes y confiables que mediante orden y cumplimiento le
			generen más soluciones y menos problemas.<br>
			<br>
			<strong>Apoyo:</strong> Además del respaldo técnico de nuestras soluciones, TEIPRO
			Colombia pondrá a su servicio a un grupo de expertos que serán parte
			de su equipo para concretar con éxito sus inventivas y resolver sus
			incidencias.<br>
			<br>
			<strong>Precios Justos:</strong> No creemos en los precios de mercado, nuestros
			proyectos son cuidadosamente calculados para ofrecer el mejor costo beneficio posible a nuestro cliente y una utilidad justa para nuestros inversionistas.		
		</div>
	</div>

</div>
@endsection